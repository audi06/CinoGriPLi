...
class cnBoxInfo(Poll, Converter, object):
	BOXTYPE = 0
	LOAD = 1
	MEMINFO = 2
	FREEFLASH = 3
	...
	def getFanInfo(self):
		try:
			with open("/proc/stb/fp/fan_speed", 'r') as file:
				fan_info = int(file.read().split()[0])
		except:
			return "Fan: N/A"
			fan_info = None
		if fan_info is not None:
			return "Fan: %s rpm" % (fan_info)			
